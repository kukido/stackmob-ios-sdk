...
describe(@"Cache Results option", ^{
    ...
    it(@"default is YES works fetches", ^{
        dispatch_group_t group = dispatch_group_create();
        dispatch_queue_t queue = dispatch_queue_create("queue", NULL);
        
        for (int i=0; i < 10; i++) {
            dispatch_group_enter(group);
            [[testProperties.client dataStore] createObject:[NSDictionary dictionaryWithObjectsAndKeys:@"todo", @"title", nil] inSchema:@"todo" options:[SMRequestOptions options] successCallbackQueue:queue failureCallbackQueue:queue onSuccess:^(NSDictionary *theObject, NSString *schema) {
                dispatch_group_leave(group);
            } onFailure:^(NSError *error, NSDictionary *theObject, NSString *schema) {
                [error shouldBeNil];
                dispatch_group_leave(group);
            }];
        }
    ...
    it(@"setting cache results to NO works fetches", ^{
        ...
        [[theValue(fetchOptions.fetchPolicySet) should] beNo];
        NSError *error = nil;
...
describe(@"Per Request Cache Policy", ^{
    ...
    it(@"setting request policy works, async", ^{
        ...
        options.fetchPolicy = SMFetchPolicyCacheOnly;
...
describe(@"Fetch with Cache", ^{
    ...
    describe(@"General Fetch Flow", ^{
        it(@"cache enabled, returned objects are saved into local cache without error", ^{
            ...
            [[theValue([testProperties.cds fetchPolicy]) should] equal:theValue(SMFetchPolicyNetworkOnly)];
...
describe(@"Purging the Cache", ^{
    ...
    it(@"Should clear the cache of objects that are deleted", ^{
        ...
        [cds setFetchPolicy:SMFetchPolicyCacheOnly];
...
describe(@"cache references should not be returned during fetches", ^{
    ...
    it(@"works", ^{
        ...
        [cds setFetchPolicy:SMFetchPolicyTryCacheElseNetwork];
